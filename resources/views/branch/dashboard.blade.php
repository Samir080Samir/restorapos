...
@section('title', 'Filial Dashboard')
...
<div class="space-y-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-slate-900">Filial Dashboard</h1>
            <p class="text-sm text-slate-500 mt-1">
                Filial üzrə satış, hesabat və statistik göstəricilər
            </p>
        </div>
        ...
        <div class="relative w-full sm:w-auto">
            <select
                ...
                class="w-full sm:w-36 h-10 rounded-xl border border-slate-200 bg-white pl-4 pr-9 text-sm text-slate-700 shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
            <div class="flex items-start justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-blue-600 mt-4">● Hazırda işlənir</p>
                </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <p class="text-sm text-slate-500">Bugünkü hesabat</p>
                    <h3 class="text-2xl font-bold mt-3 text-slate-900">24</h3>
                    <p class="text-xs text-violet-600 mt-4">● Satış əməliyyatı</p>
                </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <div class="xl:col-span-2 bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                <div>
                    <h3 class="font-bold text-slate-900 text-base">Satış strukturu</h3>
                    <p class="text-xs text-slate-500 mt-1">Filial üzrə günlük, həftəlik və aylıq satış göstəriciləri</p>
                </div>

                <div class="relative w-full sm:w-auto">
                    <select
                        style="-webkit-appearance:none; -moz-appearance:none; appearance:none; background-image:none;"
                        class="w-full sm:w-32 h-10 rounded-xl border border-slate-200 bg-slate-50 pl-3 pr-9 text-sm text-slate-700 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        <option>Bu gün</option>
                        <option>Bu həftə</option>
                        <option>Bu ay</option>
                    </select>
            ...
            <div class="h-64 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 text-sm">
                Filial satış qrafiki burada olacaq
            </div>
        </div>

        <div class="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
            <h3 class="font-bold text-slate-900 text-base">Sürətli keçidlər</h3>
            <p class="text-xs text-slate-500 mt-1 mb-4">Filial əməliyyatları üçün əsas bölmələr</p>
            ...
            <div class="space-y-3">
                ...
                <a href="#" class="flex items-center justify-between p-3.5 rounded-xl border border-slate-200 hover:bg-emerald-50 transition">
                    <span class="text-sm font-medium">Hesabatlar</span>
                ...
                <a href="#" class="flex items-center justify-between p-3.5 rounded-xl border border-slate-200 hover:bg-orange-50 transition">
                    <span class="text-sm font-medium">Statistikalar</span>
                ...
                <a href="#" class="flex items-center justify-between p-3.5 rounded-xl border border-slate-200 hover:bg-violet-50 transition">
                    <span class="text-sm font-medium">QR Menu</span>
    ...
    {{-- Filial / Hesabat / Satış / Statistika strukturu --}}
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">

        <div class="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
            <h3 class="font-bold text-slate-900 text-base">Filial</h3>
            <p class="text-xs text-slate-500 mt-1">Filialın cari iş vəziyyəti</p>
            ...
            <div class="mt-4 space-y-3 text-sm">
                ...
                <div class="flex justify-between">
                    <span class="text-slate-500">Açıq masalar</span>
                    <span class="font-medium text-slate-900">14</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-slate-500">Boş masalar</span>
                    <span class="font-medium text-slate-900">6</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
            ...
            <p class="text-xs text-slate-500 mt-1">Gündəlik filial hesabatları</p>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
            ...
            <p class="text-xs text-slate-500 mt-1">Filial performans göstəriciləri</p>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="p-4 border-b border-slate-200 flex items-center justify-between gap-3">
            <div>
                <h3 class="font-bold text-slate-900 text-base">Son filial sifarişləri</h3>
                <p class="text-xs text-slate-500 mt-1">Filial üzrə ən son satış əməliyyatları</p>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach([
                    ['id' => '#BR-1005', 'table' => 'Masa 5', 'amount' => '45.00 ₼', 'status' => 'Tamamlandı', 'color' => 'emerald', 'date' => '14.05.2025 14:35'],
                    ['id' => '#BR-1004', 'table' => 'Masa 3', 'amount' => '32.50 ₼', 'status' => 'Tamamlandı', 'color' => 'emerald', 'date' => '14.05.2025 14:20'],
                    ['id' => '#BR-1003', 'table' => 'Masa 8', 'amount' => '28.00 ₼', 'status' => 'Hazırlanır', 'color' => 'orange', 'date' => '14.05.2025 14:10'],
                    ['id' => '#BR-1002', 'table' => 'Masa 2', 'amount' => '67.00 ₼', 'status' => 'Servisdə', 'color' => 'blue', 'date' => '14.05.2025 13:55'],
                    ] as $order)
                    <tr class="hover:bg-slate-50">
                        <td class="px-4 py-3 font-medium text-slate-900">{{ $order['id'] }}</td>
                        <td class="px-4 py-3 text-slate-700">{{ $order['table'] }}</td>
                        <td class="px-4 py-3 text-slate-700">{{ $order['amount'] }}</td>
                        <td class="px-4 py-3">
