...
@section('title', 'Əməkdaşı redaktə et')
...
<div class="space-y-6">
    ...
    <div>
        <h1 class="text-2xl font-black text-slate-900">Əməkdaşı redaktə et</h1>
        <p class="text-sm text-slate-500 mt-1">
            Əməkdaş məlumatlarını, vəzifəni və giriş kodunu yeniləyin
        </p>
    </div>

    <form method="POST"
        action="{{ route('owner.staff.users.update', $user) }}"
        class="space-y-6">
        ...
        <div class="bg-white border border-slate-200 rounded-3xl p-6 space-y-5">
                ...
                <div>
                    ...
                    <select name="system_role"
                        ...
                        <option value="administrator"
                            {{ old('system_role', $user->role) == 'administrator' ? 'selected' : '' }}>
                            Administrator
                        </option>

                        <option value="cashier"
                            {{ old('system_role', $user->role) == 'cashier' ? 'selected' : '' }}>
                            Kassir
                        </option>

                        <option value="waiter"
                            {{ old('system_role', $user->role) == 'waiter' ? 'selected' : '' }}>
                            Offisiant
                        </option>

                        <option value="kitchen"
                            {{ old('system_role', $user->role) == 'kitchen' ? 'selected' : '' }}>
                            Mətbəx
                        </option>

                        <option value="finance"
                            {{ old('system_role', $user->role) == 'finance' ? 'selected' : '' }}>
                            Maliyyəçi
                        </option>

                        <option value="branch_manager"
                            {{ old('system_role', $user->role) == 'branch_manager' ? 'selected' : '' }}>
                            Filial meneceri
                        </option>

                        <option value="warehouse_manager"
                            {{ old('system_role', $user->role) == 'warehouse_manager' ? 'selected' : '' }}>
                            Anbar məsulu
                        </option>

                        <option value="staff"
                            {{ old('system_role', $user->role) == 'staff' ? 'selected' : '' }}>
                            Digər əməkdaş
                        </option>
            ...
            <div>
                ...
                <select id="ownerRoleSelect"
                    ...
                    <option value="{{ $role->id }}"
                        ...
                        {{ old('owner_role_id', $selectedRoleId) == $role->id ? 'selected' : '' }}>

                        {{ $role->name }}
                        —
                        {{ $role->login_type === 'panel' ? 'Owner panel' : 'POS ekranı' }}
            ...
            <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5">
                ...
                <div class="flex items-start justify-between gap-4">
                    ...
                    <div>
                        ...
                        <p id="codeHelp" class="text-xs text-slate-500">
                            Boş saxlasanız əvvəlki kod dəyişməyəcək.
                        </p>
                    </div>
                ...
                <input id="staffCodeInput"
                    ...
                    placeholder="Boş saxla və ya yeni kod yaz">
...
<script>
    ...
    function syncCodeHelp() {
        ...
        if (type === 'pos') {
            help.textContent =
                'POS əməkdaşları üçün 4 rəqəmli kod. Boş saxlasanız əvvəlki kod dəyişməyəcək.';
            ...
            help.textContent =
                'Owner panel əməkdaşları üçün 8 simvollu kod. Boş saxlasanız əvvəlki kod dəyişməyəcək.';
            ...
            input.placeholder = 'Boş saxla və ya yeni kod yaz';
        }
    }
    ...
    document.getElementById('ownerRoleSelect')
    ...
    document.getElementById('staffCodeInput')
