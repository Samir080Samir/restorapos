...
<div class="max-w-[1180px] mx-auto">
    ...
    <form method="POST"
        action="{{ route('owner.menu.products.update', $product) }}"
            ...
            <div class="flex items-center justify-between mb-5">
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-1">
                        POS və eMenu üçün məhsul məlumatlarını yeniləyin.
                    </p>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2 space-y-5">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select name="menu_category_id"
                                    ...
                                    <option value="{{ $category->id }}" {{ (string) old('menu_category_id', $product->menu_category_id) === (string) $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="menu_department_id"
                                    ...
                                    <option value="{{ $department->id }}" {{ (string) old('menu_department_id', $product->menu_department_id) === (string) $department->id ? 'selected' : '' }}>
                                        {{ $department->name }}
                                    </option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            ...
                            <textarea name="description"
                                ...
                                placeholder="Məhsul haqqında qısa məlumat">{{ old('description', $product->description) }}</textarea>
                        </div>
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="rounded-3xl border border-slate-200 p-4">
                            ...
                            @if($product->image)
                            <div class="mb-4 w-full h-40 rounded-2xl overflow-hidden border border-slate-200 bg-slate-100">
                            ...
                            <input type="color"
                                name="color"
                                value="{{ old('color', $product->color ?? '#4f46e5') }}"
                            ...
                            <div class="mt-4">
                                ...
                                <p class="text-xs text-slate-400 mt-2">
                                    Yeni şəkil seçilməsə, mövcud şəkil saxlanılacaq.
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="rounded-3xl border border-slate-200 p-4 space-y-4">
                            ...
                            <label class="flex items-center justify-between gap-3">
                                <span class="font-bold text-sm text-slate-700">Aktiv</span>
                                <input type="checkbox" name="is_active" value="1" class="w-5 h-5" {{ old('is_active', $product->is_active) ? 'checked' : '' }}>
                            </label>

                            <label class="flex items-center justify-between gap-3">
                                <span class="font-bold text-sm text-slate-700">Gizli</span>
                                <input type="checkbox" name="is_hidden" value="1" class="w-5 h-5" {{ old('is_hidden', $product->is_hidden) ? 'checked' : '' }}>
                            </label>

                            <label class="flex items-center justify-between gap-3">
                                <span class="font-bold text-sm text-slate-700">İkram</span>
                                <input type="checkbox" name="is_gift" value="1" class="w-5 h-5" {{ old('is_gift', $product->is_gift) ? 'checked' : '' }}>
                            </label>

                            <label class="flex items-center justify-between gap-3">
                                <span class="font-bold text-sm text-slate-700">Endirimə icazə vermə</span>
                                <input type="checkbox" name="disable_discount" value="1" class="w-5 h-5" {{ old('disable_discount', ! $product->allow_discount) ? 'checked' : '' }}>
                            </label>

                            <label class="flex items-center justify-between gap-3">
                                <span class="font-bold text-sm text-slate-700">Çəki ilə satılır</span>
                                <input type="checkbox" name="sold_by_weight" value="1" class="w-5 h-5" {{ old('sold_by_weight', $product->sold_by_weight) ? 'checked' : '' }}>
                            </label>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div class="flex items-start justify-between gap-4">
                                ...
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox"
                                        ...
                                        value="1"
                                        class="sr-only peer"
                                        {{ old('show_in_qr_menu', $product->show_in_qr_menu) ? 'checked' : '' }}>
                    ...
                    <div class="rounded-3xl border border-slate-200 p-5 bg-slate-50">
                        ...
                        <div class="mt-4 rounded-3xl bg-white border border-slate-200 p-4 shadow-sm">
                            ...
                            <div class="h-28 rounded-2xl bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center text-slate-400 text-sm font-bold overflow-hidden">
                                @if($product->image)
                                <img src="{{ asset('storage/' . $product->image) }}"
                                    class="w-full h-full object-cover">
                                @else
                            ...
                            <div class="mt-4">
                                ...
                                <div class="text-xs text-slate-500 mt-1">
                                    {{ $product->description ?: 'Məhsul açıqlaması' }}
                                </div>
